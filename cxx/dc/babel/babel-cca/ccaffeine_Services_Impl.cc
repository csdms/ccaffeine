...
#include "dc/babel/ccafe-bind/BabelOpaqueComponentFactory.hh"
...
throw ( 
  ...
  ::ccafeopq::Port* op = giz->getPort(portName);
  if(!op) {
    return gov::cca::Port(); // NULL
  }
...
is not a valid SIDL type name. A SIDL type name is a period separated list of identifiers. Each identifier must begin with a letter and then may contain any number of letters, digits and underscores. A SIDL type name should match [a-zA-Z][a-zA-Z0-9_]*(\\.[a-zA-Z][a-zA-Z0-9_]*)+"));
  ...
  ccaffeine::ports::PortTranslator px = inPort; // cast
  if(px._not_nil()) {
    ::ccafeopq::Port * op = 
...
ccaffeine::Services_impl::isPortTranslator(gov::cca::Port pq) {
  ...
  ccaffeine::ports::PortTranslator p = pq; // cast
  if(p._not_nil()) {
    return true;
  } else {
    return false;
...
//// GO PORT section //////////////////////////////////////
        ...
        )
        {
                ...
                gov::cca::ports::GoPort gp = inPort;
                if (gp._not_nil()) {
        ...
        )
        {
                ...
                ccaffeine::ports::ComponentFactory cf = inPort;
                if (cf._not_nil()) {
