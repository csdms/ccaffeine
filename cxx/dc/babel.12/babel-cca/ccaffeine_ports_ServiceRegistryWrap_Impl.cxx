...
// 
// 
#include "ccaffeine_ports_ServiceRegistryWrap_Impl.hxx"
...
throw () 
{
  // DO-NOT-DELETE splicer.begin(ccaffeine.ports.ServiceRegistryWrap.addSingletonService)
	...
	if (server._is_nil()) {
		...
		ex.SIDL_EXCEPTION_setMessage(::std::string("ccaffeine::ports::ServiceRegistryWrap_impl::addSingletonService: Port given is nil."));
#ifdef CCAFE_AUDIT
		IO_dn2("ccaffeine::ports::ServiceRegistryWrap_impl::addSingletonService: Port given is nil for type (%s)", serviceType.c_str());
#endif
	}

	if (! server.isType(serviceType)) {
		gov::cca::CCAException ex = ::ccaffeine::CCAException::_create();
		ex.SIDL_EXCEPTION_setMessage(::std::string("ccaffeine::ports::ServiceRegistryWrap_impl::addSingletonService: Port is not of type ") + serviceType);
#ifdef CCAFE_AUDIT
		IO_dn2("ccaffeine::ports::ServiceRegistryWrap_impl::addSingletonService got incorrect port type %s", serviceType.c_str());
#endif
		throw ex;
	}
