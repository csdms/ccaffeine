...
namespace {
char id[]=
"$Id: LongParameter.cxx,v 1.10 2003/07/07 19:08:40 baallan Exp $";
} ENDSEMI

LongParameter::LongParameter(char *name_, char *help_, char *prompt_, long Default_, long low_, long high_) {
  name = name_;
  Default = Default_;
  help = help_;
  prompt = prompt_;
...
int LongParameter::setValue(char *val) {
  long d = Default; // not needed if exceptions arent broken.
  int numGot;
  numGot = sscanf(val,"%ld",&d); // possible nonportability. strtol?
  if (numGot != 1 || d < low || d > high) {
    char *result;
    JCPN(StringBuffer) *sb = new JCPN(StringBuffer)((char *)"Long value {");
    sb->append(val);
...
CFREE char *LongParameter::toString() {
...
CFREE char *LongParameter::toString(char *infix) {
...
char *LongParameter::getValueString() {
  if (valueString == 0) {
